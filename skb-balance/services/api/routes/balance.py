...
async def transfer_currency(request: Request, session: Session = Depends(get_session)):
    ...
    session.add_all(_create_transactions_and_change_account_amount(from_account, to_account, float(request['amount'])))
    session.commit()
...
def _create_transactions_and_change_account_amount(
    ...
    to_account.value += amount * float(currency_cost) ** -1
